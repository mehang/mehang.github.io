...
let buttonOrder = ["#button7","#button8","#button9","#buttonDivide","#button4","#button5","#button6",
	"#buttonMultiply","#button1","#button2","#button3","#buttonAdd","#button0","#buttonClear","#buttonEquals",
	"#buttonSubtract"];

let rowIndex = 0;
let rowOrder = ["row_one", "row_two", "row_three", "row_four"];
let navItemOrder = [["#button7", "#button8", "#button9", "#buttonDivide", "#row_one"],
	["#button4", "#button5", "#button6", "#buttonMultiply", "#row_two"],
	["#button1", "#button2", "#button3", "#buttonAdd", "#row_three"],
	["#button0", "#buttonClear", "#buttonEquals", "#buttonSubtract", "#row_four"]];
...
$(document).ready(function() {
	$("#"+rowOrder[rowIndex]).addClass( "activeRow" );
	rowInterval = setInterval(navigateRowCircularly,interval);
});

function navigateRowCircularly() {
	$("#"+rowOrder[rowIndex]).removeClass("activeRow");
	rowIndex= (rowIndex + 1) % rowOrder.length;
	$("#"+rowOrder[rowIndex]).addClass("activeRow");
}

function navigateRowButtons(){
	$(navItemOrder[rowIndex][itemIndex]).removeClass("activeButton");
	itemIndex = (itemIndex + 1) % navItemOrder[rowIndex].length;
	$(navItemOrder[rowIndex][itemIndex]).addClass("activeButton");
}

function onSwitchClick() {
	if (switchRows){
		clearInterval(rowInterval);
		$(navItemOrder[rowIndex][0]).addClass("activeButton");
		itemInterval = setInterval(navigateRowButtons, interval);
	} else {
		clearInterval(itemInterval);
		$(navItemOrder[rowIndex][itemIndex]).removeClass("activeButton");
		if (navItemOrder[rowIndex][itemIndex] !== ("#"+rowOrder[rowIndex])) {
			$(navItemOrder[rowIndex][itemIndex]).click();
		}
		rowInterval = setInterval(navigateRowCircularly, interval);
		itemIndex=0;
...
$("#buttonSelect").click(onSwitchClick);
